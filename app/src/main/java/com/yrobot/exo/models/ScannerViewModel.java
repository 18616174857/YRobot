...
import com.yrobot.exo.app.MainActivity;
import com.yrobot.exo.app.ScannerFragment;
import com.yrobot.exo.app.ScannerFragmentMinimal;
...
import com.yrobot.exo.utils.SingleLiveEvent;
...
public class ScannerViewModel extends AndroidViewModel implements BleScanner.BleScannerListener {
    ...
    public void setScannerFragment(ScannerFragmentMinimal fragment) {
        mScannerFragment = fragment;
    }
    ...
    boolean checkForYrDevice1(BlePeripheral peripheral) {
        String name = peripheral.getDevice().getName();
        if (name.equals(YrConstants.BLE_NAME)) {
//            if (mScannerFragment.mIdentifier == null) {
            YrConstants.log(YrConstants.SCANNER, TAG + ".mFilteredBlePeripherals (no id) [" + name + "]");
            connectToDevice(peripheral);
//            } else {
//                if (mScannerFragment.mIdentifier.equals(peripheral.getIdentifier())) {
//                    YrConstants.log(YrConstants.SCANNER, TAG + ".mFilteredBlePeripherals (matches saved id) [" + name + "] [" + peripheral.getIdentifier() + "]");
//                    connectToDevice(peripheral);
//                }
//            }
        }
    ...
    void connectToDevice(BlePeripheral peripheral) {
        String name = peripheral.getDevice().getName();
        found_device = true;
        YrConstants.log(YrConstants.SCANNER, TAG + ".mFilteredBlePeripherals [" + name + "]");
        if (peripheral.getConnectionState() == BlePeripheral.STATE_DISCONNECTED) {
            peripheral.connect(null);
            YrConstants.log(YrConstants.SCANNER, TAG + ".mFilteredBlePeripherals \n[" + name + "] Connect\n");
        }
    }

    boolean useRssiLimit = true;
//    int minRssi = -60;
//    int minRssi = -75;
    int minRssi = -150;

    boolean checkForYrDevice(List<BlePeripheral> results) {
        ArrayList<BlePeripheral> matches = new ArrayList<>();
        for (Iterator<BlePeripheral> it = results.iterator(); it.hasNext(); ) {
            BlePeripheral ble_peripheral = it.next();
            String name = ble_peripheral.getDevice().getName();
            if (name.equals(YrConstants.BLE_NAME)) {
                matches.add(ble_peripheral);
                YrConstants.log(YrConstants.SCANNER, TAG + ".mFilteredBlePeripherals (no id) [" + name + "]");
            }
        }
        ...
        if (matches.size() > 0) {
            String tag = "yr-sort";
            for (BlePeripheral p : matches) {
                Log.v(tag, "[" + p.getIdentifier() + "] RSSI [" + p.getRssi() + "] [" + p.getLastRssi() + "]");
            }
            if (matches.size() > 1) {
                Collections.sort(matches, new Comparator<BlePeripheral>() {
                    public int compare(BlePeripheral obj1, BlePeripheral obj2) {
                        return Integer.valueOf(obj2.getRssi()).compareTo(Integer.valueOf(obj1.getRssi()));
                    }
                });

                Log.v(tag, "After sorting");
                for (BlePeripheral p : matches) {
                    Log.v(tag, "[" + p.getIdentifier() + "] RSSI [" + p.getRssi() + "] [" + p.getLastRssi() + "]");
                }
            }

            BlePeripheral selected = matches.get(0);

            boolean signalStrengthTooLow = selected.getRssi() < minRssi;

            Log.v(tag, "Connect to [" + selected.getIdentifier() + "]");
            Log.v(tag, "Over RSSI threshold [" + !signalStrengthTooLow + "]");

            if (!useRssiLimit || (useRssiLimit && !signalStrengthTooLow)) {
...
//            BlePeripheral ble_peripheral = it.next();
//            String name = ble_peripheral.getDevice().getName();
//            checkForYrDevice1(ble_peripheral);
//        }

        checkForYrDevice(results);
    ...
    public void setDefaultFilters(boolean shouldRestoreSavedValues) {
        ...
        YrConstants.log(YrConstants.SCANNER, "setDefaultFilters [" + shouldRestoreSavedValues + "]");

        filterData.name = YrConstants.BLE_NAME;
    ...
    public void start() {
        if (!mScanner.isScanning()) {
            ...
            YrConstants.log(YrConstants.SCANNER, "Started scanning");
            mScanner.start();
        } else {
    ...
    public void stop() {
        if (mScanner.isScanning()) {
            ...
            YrConstants.log(YrConstants.SCANNER, "Stop scanning");
            mScanner.stop();
        }
    }

    public void setNameFilter(String name) {
        YrConstants.log(YrConstants.SCANNER, "Set name filter [" + name + "]");
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (blePeripheral != null) {
                if (BlePeripheral.kBlePeripheral_OnConnected.equals(action)) {
                    ...
                    YrConstants.log(YrConstants.SCANNER, TAG + ".onReceive: onConnected [" + identifier + "]");
                        ...
                        final Runnable discoveredServicesRunnable = () -> {
                            ...
                            if (status == BluetoothGatt.GATT_SUCCESS) {
                                ...
                                if (mScannerFragment != null) {
                                    mScannerFragment.onConnectedCallback(blePeripheral);
                                }
                    ...
                    YrConstants.log(YrConstants.SCANNER, TAG + ".onReceive: onDisconnected [" + identifier + "]");
                    ...
                    YrConstants.log(YrConstants.SCANNER, "onReceive: onConnecting [" + identifier + "]");
